...
class FlowVariable(Base):
    ...
    is_runtime_override_magic = Column(
        Boolean, default=False
    )  # LLM to magically get it from the user context
    runtime_override_action_id = Column(
        String(255), default=False, nullable=True
    )  # In case the variable value will be assigned during the run from previous action response

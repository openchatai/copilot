        ...
        <Draggable draggableId={"ACTION|" + action.id} index={index} disableInteractiveElementBlocking>
            {draggableInPortal((provided, snapshot) => {
                ...
                return <React.Fragment>
                    <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} style={getStyle(provided.draggableProps.style, snapshot)} className="w-full transition-all">
...
export function ActionsList({ disabled }: { disabled?: boolean }) {
    ...
    return (
        _.isEmpty(actions) ? <EmptyBlock className="h-full text-xs">
            ...
            <Droppable isDropDisabled={disabled} droppableId={ASIDE_DROPABLE_ID} mode="standard">
                {
                    (provided) => {
